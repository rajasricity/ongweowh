...
class Common extends CI_Model{
	...
	public function getInventorycount($database,$table,$aid,$loccode,$column,$itemname){
		...
		$data = $this->mongo_db->get_where($table,["appId"=>$aid,$column=>$loccode,"item"=>$itemname]);
	...
	public function getFiltervalues($cursor,$table){
		...
		foreach($cursor as $erow){
			...
			if($table == "tbl_locations"){
				...
				$itemstatus = $this->mongo_db->get_where("tbl_items",["item_name"=>$row["item"]])[0];
				...
				if($itemstatus["status"] == "Active" && $flocstatus["status"] == "Active" && $tlocstatus["status"] == "Active"){
				...
				if($itemstatus["status"] == "Active" && $tlocstatus["status"] == "Active"){
				
					$shipmentdate=$this->common->getConverteddate($row["shippmentdate"]);
					$chepprocessdate=$this->common->getConverteddate($row["chepprocessdate"]);
				
					$row["check"] = '<input type="checkbox" class="check" name="lid" value="'.$row["_id"]{'$oid'}.'">';
					$row["Actions"] = '<a href="javascript:void(0)" class="editLocate" id="'.$row["_id"]->{'$id'}.'" chepreference="'.$row["chepreference"].'" ongreference="'.$row["ongreference"].'" shippmentdate="'.$shipmentdate.'" quantity="'.$row["quantity"].'" item="'.$row["item"].'" tlcoation="'.$row["tlocation"].'" tlcoationcode="'.$row["tlcoationcode"].'" chepprocessdate="'.$chepprocessdate.'" umi="'.$row["umi"].'"><i class="far fa-edit"></i></a>&nbsp;&nbsp;<a href="javascript:void(0)" id="'.$row["_id"]{'$oid'}.'" onclick="archiveFunction(this.id)" class="delItem"><i class="fa fa-trash" style="color:red"></i></a>';
				...
				if($itemstatus["status"] == "Active" && $tlocstatus["status"] == "Active"){
					...
					$row["check"] = '<input type="checkbox" class="check" name="lid" value="'.$row["_id"]{'$oid'}.'">';
					$row["Actions"] = '<a href="javascript:void(0)" class="editLocate" id="'.$row["_id"]->{'$id'}.'" chepreference="'.$row["chepreference"].'" ongreference="'.$row["ongreference"].'" shippmentdate="'.$shipmentdate.'" quantity="'.$row["quantity"].'" item="'.$row["item"].'" tlocation1="'.$row["tlocation"].'" tlcoationcode="'.$row["tlcoationcode"].'" chepprocessdate="'.$chepprocessdate.'" umi="'.$row["umi"].'"><i class="far fa-edit"></i></a>&nbsp;&nbsp;<a href="javascript:void(0)" id="'.$row["_id"]{'$oid'}.'" onclick="archiveFunction(this.id)" class="delItem"><i class="fa fa-trash" style="color:red"></i></a>';
				...
				if($itemstatus["status"] == "Active" && $tlocstatus["status"] == "Active"){	
					...
					$row["check"] = '<input type="checkbox" class="check" name="lid" value="'.$row["_id"]{'$oid'}.'">';
					$row["Actions"] = '<a href="javascript:void(0)" class="editLocate" lid="'.$row["_id"]->{'$id'}.'" chepreference="'.$row["chepreference"].'" ongreference="'.$row["ongreference"].'" shippmentdate="'.$shipmentdate.'" quantity="'.$row["quantity"].'" item="'.$row["item"].'" tlocation="'.$row["tlocation"].'" chepprocessdate="'.$chepprocessdate.'" adjdirection="'.strtoupper($row["adjdirection"]).'" umi="'.$row["umi"].'" tlcoationcode="'.$row["tlcoationcode"].'"><i class="far fa-edit"></i></a>&nbsp;&nbsp;<a href="javascript:void(0)" id="'.$row["_id"]{'$oid'}.'" onclick="archiveFunction(this.id)" class="delItem"><i class="fa fa-trash" style="color:red"></i></a>';
	...
	public function getConditionbydatatype($datatype){
		
		if($datatype == "text" || $datatype == "textarea" || $datatype == "select"){
...
//		echo json_encode($out);
	...
	public function getUpdatedFieldsOperators($col="",$dType="",$tbl="",$soperator="",$sfield="",$refopid="",$cnameref=""){
		...
		}elseif($column == "starting_balance" || $column == "issues" || $column == "returns" || $column == "transfer_ins" || $column == "transfer_outs" || $column == "adjustments" || $column == "ending_balance" || $column == "audit_count2019" || $column == "quantity"){
		...
		$oper = '<select name="condition'.$cnameref.'[]" class="form-control updateonchangeCondition" rCount="'.$cnameref.'" opid="'.$refopid.'">';
	...
	public function getsetvalfields($tbl="",$val=""){
		...
		foreach($lcolumns->labels as $key => $labels){
			...
			$selected = ($lcolumns->columns[$key]==$val) ? 'selected' : '';
			
			$columns .= '<option value="'.$lcolumns->columns[$key].'" '.$selected.'>'.$labels.'</option>';
		...
		if($column == "locname" && $table != "tbl_locations"){
			...
			$accounts .= '</select>';
		}elseif($column == "starting_balance" || $column == "issues" || $column == "returns" || $column == "transfer_ins" || $column == "transfer_outs" || $column == "adjustments" || $column == "ending_balance" || $column == "audit_count2019"){
...
//			$finish = (date('D') != 'Sun') ? date('Y-m-d', strtotime('next Sunday')) : date('Y-m-d');
    ...
    public function getUpdatedFieldsOperatorsUpdated($col="",$dType="",$tbl="",$soperator="",$sfield="",$refopid="",$cnameref=""){
		...
		if($column == "locname" && $table != "tbl_locations"){
			...
			$locations = $this->mongo_db->get($table);
			
			$locnames = "";
			
			foreach($locations as $loc){

				$locnames .= '<option value='.$loc["_id"]->{'$id'}.'>'.$loc['locname'].'</option>';
